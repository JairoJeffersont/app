...
class ProposicaoModel
{
    ...
    {
        $query = "SELECT proposicao_tipo, proposicao_ano, COUNT(*) as total 
              FROM view_proposicoes 
              WHERE proposicao_autor_nome = :autor AND
                    proposicao_autor_proponente = 1 AND
                    proposicao_autor_assinatura = 1 
              GROUP BY proposicao_tipo, proposicao_ano
              ORDER BY proposicao_ano DESC, total DESC";
