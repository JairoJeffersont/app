        ...
        <div class="container-fluid p-2">
            ...
            <div class="card mb-2 no-print">
                <div class="card-body p-1">
                    ...
                    <a class="btn btn-success btn-sm custom-nav card-description" href="?secao=emendas" role="button"><i class="bi bi-arrow-left"></i> Voltar</a>
                ...
                <div class="card-body p-2">
                    ...
                    <form class="row g-2 form_custom no-print" id="form_novo" method="POST" enctype="multipart/form-data">
                        <div class="col-md-2 col-12">
                            <input type="text" class="form-control form-control-sm" name="emenda_numero" placeholder="Número da Emenda" value="<?php echo $buscaEmenda['dados'][0]['emenda_numero'] ?>" required>
                        </div>
                        <div class="col-md-2 col-12">
                            <input type="text" class="form-control form-control-sm" name="emenda_valor" id="emenda_valor" placeholder="Valor da Emenda (R$)" value="<?php echo $buscaEmenda['dados'][0]['emenda_valor'] ?>" required>
                        </div>
                        <div class="col-md-8 col-12">
                            <input type="text" class="form-control form-control-sm" name="emenda_descricao" placeholder="Descrição" value="<?php echo $buscaEmenda['dados'][0]['emenda_descricao'] ?>" required>
                        </div>
                            ...
                            <select class="form-select form-select-sm" name="emenda_status" id="emenda_status" required>
                                ...
                                if ($emendasStatus['status'] == 'success') {
                                    foreach ($emendasStatus['dados'] as $status) {
                                        if ($status['emendas_status_id'] == 1 || $status['emendas_status_id'] == $buscaEmenda['dados'][0]['emenda_status']) {
                            ...
                            <select class="form-select form-select-sm" name="emenda_orgao" id="orgao" required>
                                ...
                                if ($orgaos['status'] == 'success') {
                                    foreach ($orgaos['dados'] as $orgao) {
                                        if ($orgao['orgao_id'] == 1 || $orgao['orgao_id'] == $buscaEmenda['dados'][0]['emenda_orgao']) {
                            ...
                            <select class="form-select form-select-sm" name="emenda_objetivo" id="emenda_objetivo" required>
                                ...
                                if ($emendasObjetivos['status'] == 'success') {
                                    foreach ($emendasObjetivos['dados'] as $objetivo) {
                                        if ($objetivo['emendas_objetivos_id'] == 1 || $objetivo['emendas_objetivos_id'] ==  $buscaEmenda['dados'][0]['emenda_objetivo']) {
                        ...
                        <div class="col-md-6 col-12">
                            <select class="form-select form-select-sm" name="emenda_tipo" required>
                                <option value="1" <?= ($buscaEmenda['dados'][0]['emenda_tipo'] == 1) ? 'selected' : ''; ?>>Emenda parlamentar</option>
                                <option value="2" <?= ($buscaEmenda['dados'][0]['emenda_tipo'] == 2) ? 'selected' : ''; ?>>Emenda de bancada</option>
                            </select>
                        </div>

                        <div class="col-md-12 col-12">
                            <textarea class="form-control form-control-sm" name="emenda_informacoes" placeholder="Informações Adicionais. Ex. Ordem de pagamento, códigos gerais..." rows="5" required><?php echo $buscaEmenda['dados'][0]['emenda_informacoes'] ?></textarea>
                        </div>
...
<script>
    ...
    function carregarEstados() {
        $.getJSON('https://servicodados.ibge.gov.br/api/v1/localidades/estados?orderBy=nome', function(data) {
            ...
            data.forEach(estado => {
                if (estado.sigla === "<?php echo $buscaEmenda['dados'][0]['emenda_estado'] ?>") {
                    setTimeout(function() {
    ...
    function carregarMunicipios(estadoId) {
        $.getJSON(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${estadoId}/municipios?orderBy=nome`, function(data) {
            ...
            data.forEach(municipio => {
                if (municipio.nome === "<?php echo $buscaEmenda['dados'][0]['emenda_municipio'] ?>") {
